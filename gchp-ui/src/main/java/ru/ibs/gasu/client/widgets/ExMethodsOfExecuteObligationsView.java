...
import ru.ibs.gasu.common.models.ExEnsureMethodModel;
...
public class ExMethodsOfExecuteObligationsView implements IsWidget {

    private VerticalLayoutContainer container;
    private Grid<ExEnsureMethodModel> grid;

    public Grid<ExEnsureMethodModel> getGrid() {
        return grid;
    }

    public ExMethodsOfExecuteObligationsView() {
        initWidget();
    }


    public boolean containsMethod(Long methodId) {
        for (ExEnsureMethodModel model : grid.getStore().getAll()) {
    ...
    private void initWidget() {
        ...
        ListStore<ExEnsureMethodModel> store = new ListStore<>(new ModelKeyProvider<ExEnsureMethodModel>() {
            @Override
            public String getKey(ExEnsureMethodModel item) {
                return String.valueOf(item.getGid());
            }
        });

        ColumnConfig<ExEnsureMethodModel, ExEnsureMethodModel> methodType = new ColumnConfig<>(new ValueProvider<ExEnsureMethodModel, ExEnsureMethodModel>() {
            @Override
            public ExEnsureMethodModel getValue(ExEnsureMethodModel object) {
                return object;
            }

            @Override
            public void setValue(ExEnsureMethodModel object, ExEnsureMethodModel value) {
            }
        ...
        ActionCell.Delegate<ExEnsureMethodModel> methodTypeDelegate = new ActionCell.Delegate<ExEnsureMethodModel>() {
            @Override
            public void execute(ExEnsureMethodModel object) {
                new ExMethodsOfExeObligationDicWindow() {
                    @Override
        ...
        methodType.setCell(new ActionCell<ExEnsureMethodModel>("", methodTypeDelegate) {
            @Override
            public void render(Context context, ExEnsureMethodModel value, SafeHtmlBuilder sb) {
                sb.appendHtmlConstant("<span style='cursor: pointer;'>");
                if (value == null || value.getEnsureMethodName() == null)
                    sb.appendHtmlConstant("Выберите способ обеспечения");
                else
        ...
        ColumnConfig<ExEnsureMethodModel, String> riskType = new ColumnConfig<>(new ValueProvider<ExEnsureMethodModel, String>() {
            @Override
            public String getValue(ExEnsureMethodModel object) {
            ...
            @Override
            public void setValue(ExEnsureMethodModel object, String value) {
        ...
        ColumnConfig<ExEnsureMethodModel, String> info = new ColumnConfig<>(new ValueProvider<ExEnsureMethodModel, String>() {
            @Override
            public String getValue(ExEnsureMethodModel object) {
        ...
        ColumnConfig<ExEnsureMethodModel, String> date = new ColumnConfig<>(new ValueProvider<ExEnsureMethodModel, String>() {
            @Override
            public String getValue(ExEnsureMethodModel object) {
                //return WidgetUtils.formatDate(object.getTerm(), "dd.MM.yyyy");
        ...
        ColumnConfig<ExEnsureMethodModel, String> submissionDate = new ColumnConfig<>(new ValueProvider<ExEnsureMethodModel, String>() {
            @Override
            public String getValue(ExEnsureMethodModel object) {
        ...
        ColumnConfig<ExEnsureMethodModel, String> sum = new ColumnConfig<>(new ValueProvider<ExEnsureMethodModel, String>() {
            @Override
            public String getValue(ExEnsureMethodModel object) {
        ...
        ColumnModel<ExEnsureMethodModel> cm = new ColumnModel<>(Arrays.asList(methodType, info, date, sum));
        ...
        final GridInlineEditing<ExEnsureMethodModel> editing = new GridInlineEditing<ExEnsureMethodModel>(grid) {
            @Override
            protected void onScroll(ScrollEvent event) {
            }
        };
        editing.setClicksToEdit(ClicksToEdit.ONE);
        //editing.addEditor(date, createStringDateConverter(), new TextField());
        ...
        grid.getStore().addSortInfo(new Store.StoreSortInfo<ExEnsureMethodModel>(new Comparator<ExEnsureMethodModel>() {
            @Override
            public int compare(ExEnsureMethodModel o1, ExEnsureMethodModel o2) {
        ...
        addEventButton.addSelectHandler(new SelectEvent.SelectHandler() {
            ...
            public void onSelect(SelectEvent event) {
                ExEnsureMethodModel eventModel = new ExEnsureMethodModel();
        ...
        editing.addBeforeStartEditHandler(new BeforeStartEditEvent.BeforeStartEditHandler<ExEnsureMethodModel>() {
            @Override
            public void onBeforeStartEdit(BeforeStartEditEvent<ExEnsureMethodModel> event) {
                ExEnsureMethodModel selectedItem = event.getSource().getEditableGrid()
