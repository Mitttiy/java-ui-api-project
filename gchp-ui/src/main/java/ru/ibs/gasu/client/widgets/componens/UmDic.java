...
public class UmDic extends VerticalLayoutContainer implements IsField<List<UmModel>> {
    ...
    public static class MultiFieldAppearance extends Css3ValueBaseFieldAppearance implements TriggerFieldCell.TriggerFieldAppearance {
        ...
        public MultiFieldAppearance(Css3TriggerFieldResources resources) {
            super(resources);
    ...
    interface Templates extends XTemplates {
        @XTemplate("<h3 style='font-family: \"Roboto\", sans-serif;'>Выбор единицы измерения</h3>")
        SafeHtml header();
    ...
    private List<UmModel> values = new ArrayList<>();

    private StoreFilterField<String> searchText;
    ...
    private void createWindow() {
        ...
        CloseButton closeButton = new CloseButton() {
            ...
            protected void onClick(Event event) {
                ...
                ValueChangeEvent.fire(UmDic.this, oldValues);
                window.hide();
            }
        };
        ...
        searchText = new StoreFilterField<String>() {
            @Override
        ...
        searchText.setBorders(false);
        searchText.setEmptyText("Введите ОКЕИ или наименование для поиска...");
        container.add(searchText, new VerticalLayoutData(1, -1, new Margins(5, 5, 10, 10)));
        ...
        TextButton apply = new TextButton() {
            ...
            protected void onClick(Event event) {
                onSelectionChange();
                ValueChangeEvent.fire(UmDic.this, getSelectedItems());
                window.hide();
            }
        };
        apply.setWidth(100);
        ...
        clear.setWidth(100);
        clear.getCell().setHTML(templates.button("Сбросить", "fa-eraser"));
        clear.addStyleName("x-toolbar-mark");

        buttonContainer.add(apply, new BoxLayoutContainer.BoxLayoutData(new Margins(0, 15, 0, 0)));
        buttonContainer.add(clear, new BoxLayoutContainer.BoxLayoutData(new Margins(0, 15, 0, 0)));
    ...
    private MultiComboBoxTriggerField selectedItemsField = new MultiComboBoxTriggerField(new TriggerFieldCell<>(GWT.<MultiFieldAppearance>create(MultiFieldAppearance.class))) {
        @Override
    ...
    public UmDic(Style.SelectionMode selectionMode, boolean isShowOnAttach) {
        this(selectionMode, isShowOnAttach, selectionMode == Style.SelectionMode.MULTI);
    }

    public UmDic(Style.SelectionMode selectionMode, boolean isShowOnAttach, boolean showTextValues) {
    ...
    private Grid<UmModel> grid;
    private List<ColumnConfig<UmModel, ?>> columns;
    private VerticalLayoutContainer container;
    private PagingLoader<FilterPagingLoadConfig, PagingLoadResult<UmModel>> loader;
    private ListStore<UmModel> store;
    ...
    private void selectStore() {
        if (values.size() > 0) {
            for (UmModel participant : values) {
                select(participant);
            }
        }
    }
    ...
    private VerticalLayoutContainer getGrid() {
        ...
        RpcProxy<FilterPagingLoadConfig, PagingLoadResult<UmModel>> rpcProxy =
                new RpcProxy<FilterPagingLoadConfig, PagingLoadResult<UmModel>>() {
                    @Override
                    public void load(FilterPagingLoadConfig loadConfig, AsyncCallback<PagingLoadResult<UmModel>> callback) {
                        UmSearchCriteria umSearchCriteria = new UmSearchCriteria();
                        umSearchCriteria.setLimit(loadConfig.getLimit());
                        umSearchCriteria.setOffset(loadConfig.getOffset());
                        if (searchText != null && searchText.getText() != null && searchText.getText().length() > 0) {
                            umSearchCriteria.setIdOrName(searchText.getText());
                        }
                        WindowDictsApi.WINDOW_DICTS_API.getFilteredUnitsOfMeasurement(umSearchCriteria, new DefaultCallback(callback));
                    }
                };

        loader = new PagingLoader<FilterPagingLoadConfig, PagingLoadResult<UmModel>>(rpcProxy) {
            @Override
        ...
        loader.addBeforeLoadHandler(event -> loading = true);
        loader.addLoadExceptionHandler(event -> loading = false);
        loader.addLoadHandler(new LoadResultListStoreBinding<FilterPagingLoadConfig, UmModel, PagingLoadResult<UmModel>>(store) {
            @Override
    ...
    private ColumnModel<UmModel> getColumnModel() {

        ColumnConfig<UmModel, String> idColumn = new ColumnConfig<>(new ValueProvider<UmModel, String>() {
            @Override
            public String getValue(UmModel object) {
                return object.getOkeiCode();
            }

            @Override
            public void setValue(UmModel object, String value) {
        ...
        ColumnConfig<UmModel, String> nameColumn = new ColumnConfig<>(new ValueProvider<UmModel, String>() {
            @Override
            public String getValue(UmModel object) {
        ...
        }, 500, WidgetUtils.wrapString("Наименование"));
        nameColumn.setMenuDisabled(true);
        nameColumn.setSortable(false);

        IdentityValueProvider<UmModel> identity = new IdentityValueProvider<UmModel>();
        selectionModel = new CheckBoxSelectionModel<UmModel>(identity) {
            @Override
    ...
    public void select(UmModel item) {
        select(Long.toString(item.getId()));
    }

    public void selectAll(List<UmModel> items) {
        List<UmModel> selected = new ArrayList<>(items);
        selectionModel.deselectAll();
    ...
    public void select(String key) {
        UmModel item = store.findModelWithKey(key);
        if (item != null && !selectionModel.isSelected(item)) {
            selectionModel.select(true, item);
        }
    }

    private CheckBoxSelectionModel<UmModel> selectionModel;
    ...
    private void updateSelection() {
        if (selectionMode == Style.SelectionMode.SINGLE) {
            ...
            Map<Long, UmModel> prevSelection = values.stream().collect(Collectors.toMap(item -> item.getId(), item -> item));
            for (UmModel item : grid.getStore().getAll()) {
    ...
    public void controlSelection(UmModel item) {
        if (item != null) {
    ...
    public List<UmModel> getSelectedItems() {
        return values;
    }

    public UmModel getFirstValue() {
        if (values != null && values.size() > 0) return values.get(0);
        return null;
    }

    public void setSingleValue(UmModel umDomain) {
        if (umDomain != null) {
            values.clear();
            values.add(umDomain);
            onSelectionChange();
    ...
    @Override
    public HandlerRegistration addValueChangeHandler(ValueChangeHandler<List<UmModel>> handler) {
        return addHandler(handler, ValueChangeEvent.getType());
    }

    private List<UmModel> oldValues = new ArrayList<>();

    @Override
    public void setValue(List<UmModel> value) {
