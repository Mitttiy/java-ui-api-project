...
function FillCertList_Async(lstId) {
    ...
    cadesplugin.async_spawn(function *() {
        ...
        try {
		    ...
		    if (!oStore) {
                callback.initCompleteFailure("store failed");
		        return;
		    }
...
*/
        ...
        for (var i = 1; i <= certCnt; i++) {
            ...
            try {
                ...
                var snilsArr = getSnilsArr(yield cert.SubjectName);
                ...
                if(dateObj< ValidToDate && (yield cert.HasPrivateKey()) && IsValid) {
                    ...
                    var certString = new CertificateAdjuster().GetCertInfoString(yield cert.SubjectName, ValidFromDate);

                    certNameList.push(certString);
                    certThumbprintList.push(yield cert.Thumbprint);
                    ...
                    //
                    var metaData = new certMetaData(yield cert.SubjectName, Adjust.GetIssuer(yield cert.IssuerName), Adjust.GetCertDate(yield cert.ValidFromDate), Adjust.GetCertDate(yield cert.ValidToDate), sProviderName, fAlgoName, snils);
...
//                console.log("Thumbprint: " + (yield cert.Thumbprint));
            }
            catch (ex) {
...
function SignCadesBES_Async(thumbprint, data, setDisplayData) {
    cadesplugin.async_spawn(function*(arg) {
        ...
        try {
            ...
            callback.signCompleteFailure('Failed to create CAdESCOM.Store: ' + err.number);
            return;
        }
        ...
        if ((yield oCerts.Count) == 0) {
            callback.signCompleteFailure("Certificate not found");
            return;
        }
...
//            console.log("Плагин загружен, но он не поддерживает УЭК.");
        }
        else
        {
            //document.getElementById('PluginEnabledImg').setAttribute("src", "Img/green_dot.png");
...
//                console.log("Плагин загружен. Не установлен УЭК CSP.");
            }
        }
        //document.getElementById('PlugInVersionTxt').innerHTML = "Версия плагина: " + (yield CurrentPluginVersion.toString());
        //document.getElementById('CSPVersionTxt').innerHTML = "Версия УЭК CSP: " + (yield UECCSPVersion.toString());
//        console.log("Версия плагина: " + (yield CurrentPluginVersion.toString()));
//        console.log("Версия УЭК CSP: " + (yield UECCSPVersion.toString()));
    }); //cadesplugin.async_spawn
}
            ...
            cadesplugin.async_spawn(function *(args) {
                try {
                    ...
                    if(typeof oCertTemp == "undefined")
                    {
                        ...
                        console.log("Сертификат не найден или отсутствует.");
                    ...
                    if(yield FoundCertInStore_Async(oCertTemp)) {
                        ...
                        console.log("Сертификат не найден в хранилище MY.");
                    ...
                    if("The action was cancelled by the user. (0x8010006E)" == message) {
                        ...
                        console.log("Карта не найдена или отсутствует сертификат на карте.");
                    }
                    args[1]();
                }
            }, resolve, reject);
